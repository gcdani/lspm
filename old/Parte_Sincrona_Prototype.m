...
% Zdf_menos = Zmd_menos -R/3 + 1i*X1d;
% Zdb_menos = Zmd_menos - R + 1i*X1d;
% Zqf_menos = Zmq_menos -R/3 + 1i*X1d;
% Zqb_menos = Zmq_menos -R + 1i*X1d;
...
for int = 1:length(delta_carga)
    ...
    M = [H, G;F, J];
    Vpk = sqrt(2).*[abs(V_alpha_beta(1)); -1i*abs(V_alpha_beta(2))];
    
    I_d_q = M\Vpk;
    
    P_in(int) = real(Vs*sum(I_m_a));
    T_rel(int) = P*(Xd - Xq)*real(Im(1))*imag(Im(2))/w;
    ...
    T_tras(int) = P_tras/w;
    Tns(int) = P*(real(V2n*conj(I2n)) - R*I2n*I2n)/w;
    Wcur(int) = 2*real(V2n*conj(I2n)) - P_cu_tras(int);
    Wcur2(int) = 2*w*Tns(int);
    
    P_out(int) = P_in(int) - Wcur(int) - P_ferro;
    
    T_total(int) = T_frente(int) + Tns(int);
